...
class Vocab(object):
    def __init__(self, vocab_file=None, vocab_size=50000):
        ...
        for w in [PAD_TOKEN, UNKNOWN_TOKEN, START_DECODING, STOP_DECODING, UTTERENCE_END]:
...
def makeVocabdict(data_dir, vocab_dir, vocab_size):
    ...
    queries = df['answers'].tolist()
    articles = df['text'].tolist()
    titles = df['questions'].tolist()



    for ques, art, tit, in tqdm(zip(queries, articles, titles)):
        try:
            ''' #for CNN/DM 
            ...
            art_ = [a.replace('\r', '') for a in art_]
            art_ = ' '.join(art_)
            ...
            art_ = ques+"|"+art
            art_ = art_.replace('|', ' ')
            # art_ = art_.replace('|', ' ').replace('技师说：', '').replace('车主说：', '')
            # print(art_)
            # exit()

            art_tokens = jieba.lcut(art_)
            tit_tokens = jieba.lcut(tit)

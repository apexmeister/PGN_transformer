...
#     def __init__(self, hidden_dim, n_heads, dropout, use_utter_trunc=False, use_user_mask=False):
...
#         if self.use_utter_trunc or self.use_user_mask:
#             context_layer = self._forward_with_utter_mask(hidden_states, attention_mask)
#         else:
#             context_layer = self._forward(hidden_states, attention_mask)
...
#         return context_layer
#
#     def _forward_with_utter_mask(self, hidden_states, attention_mask):
#         mixed_query_layer = self.query(hidden_states)
#         mixed_key_layer = self.key(hidden_states)
#         mixed_value_layer = self.value(hidden_states)
#
...
#     def __init__(self, hidden_dim, n_heads, dropout, eps, use_utter_trunc=False, use_user_mask=False):
#         super(BertAttention, self).__init__()
#         self.self = BertSelfAttention(hidden_dim, n_heads, dropout, use_utter_trunc)
...
#     def __init__(self, hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc=False, use_user_mask=False):
#         super(BertLayer, self).__init__()
#         self.attention = BertAttention(hidden_dim, n_heads, dropout, eps, use_utter_trunc, use_user_mask)
...
#                  pad_idx, max_article_len, dropout, eps, use_utter_trunc=False, use_user_mask=False):
#         super(Encoder, self).__init__()
#
#         self.use_utter_trunc = use_utter_trunc
#         self.use_user_mask = use_user_mask
#
#         if self.use_utter_trunc:
#             self.utter_bertLayer = BertLayer(hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc,
#                                        use_user_mask)
#         if self.use_user_mask:
#             self.user_bertLayer = BertLayer(hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc,
#                                        use_user_mask)
#
#         if self.use_utter_trunc and self.use_user_mask:
...
#         self.bertLayer = BertLayer(hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc, use_user_mask)
...
#     def forward(self, encoder_input, encoder_mask, utter_mask=None, user_mask=None):
#         embedded = self.dropout(self.embedding(encoder_input))  # 这里的dropout可以不用加了
#         if self.use_utter_trunc and self.use_user_mask:
#             utter_bert_output = self.utter_bertLayer(hidden_states=embedded, attention_mask=utter_mask)
#             user_bert_output = self.user_bertLayer(hidden_states=embedded, attention_mask=user_mask)
#             bert_output = torch.cat((utter_bert_output, user_bert_output), dim=-1)
#             bert_output = self.fusion_gate(bert_output)
#         elif self.use_utter_trunc and not self.use_user_mask:
#             bert_output = self.utter_bertLayer(hidden_states=embedded, attention_mask=utter_mask)
#         elif not self.use_utter_trunc and self.use_user_mask:
#             bert_output = self.user_bertLayer(hidden_states=embedded, attention_mask=user_mask)
#         else:
#             bert_output = self.bertLayer(hidden_states=embedded, attention_mask=encoder_mask)
#         # print(bert_output.size())
#         # print(embedded.size())
#         # exit()
#
#         bool_mask = (encoder_mask == 0)
#         # print(bool_mask)
#         # 在mask的位置用value来填充，
#         bert_output_masked = bert_output.masked_fill(mask=bool_mask.unsqueeze(dim=2), value=0)
#
#         seq_lens = encoder_mask.sum(dim=-1)  # 统计每个句子的长度，【batch ?
#         # print(encoder_mask.size())
#         # print(bert_output_masked.size())
#         # print(seq_lens)
...
#                  intermediate_dim, dropout, eps, use_pointer, use_coverage, use_utter_trunc=False):
#         super(Decoder, self).__init__()
#
#         self.bertLayer = BertLayer(hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc)
#         # self.bertLayer_ = BertLayer(hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc)
#         # self.bertLayer__ = BertLayer(hidden_dim, intermediate_dim, n_heads, dropout, eps, use_utter_trunc)
#
...
#         # bert_output = self.bertLayer_(hidden_states=bert_output, attention_mask=decoder_mask_one_step)
#         # bert_output = self.bertLayer__(hidden_states=bert_output, attention_mask=decoder_mask_one_step)
...
#                                    opt.pad_idx, opt.max_article_len, opt.dropout, opt.eps, opt.use_utter_trunc, opt.use_user_mask)
...
#         self.use_utter_trunc = opt.use_utter_trunc
#         self.use_user_mask = opt.use_user_mask
#
...
#                 decoder_input=None, decoder_mask=None, decoder_target=None, utter_mask=None, user_mask=None,
...
#                                        coverage, decoder_input, decoder_mask, decoder_target, utter_mask, user_mask)
#             # print(model_loss)
#
#             return model_loss
#         elif mode in ["decode"]:
...
#                                  coverage=coverage, utter_mask=utter_mask, user_mask=user_mask, beam_size=beam_size)
#
#     def _forward(self, encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec, coverage,
#                  decoder_input, decoder_mask, decoder_target, utter_mask=None, user_mask=None):
#
#         assert isinstance(self.encoder, Encoder)
#         if self.use_utter_trunc or self.use_user_mask:
#             encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=utter_mask, encoder_mask=encoder_mask, user_mask=user_mask)
#         else:
#             encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=encoder_mask, encoder_mask=encoder_mask, user_mask=user_mask)
#
...
#         return result_loss
#
#     def _decoder(self, encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec, coverage, utter_mask=None, user_mask=None, beam_size=4):
#         if self.use_utter_trunc or self.use_user_mask:
#             encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=utter_mask, encoder_mask=encoder_mask, user_mask=user_mask)
#         else:
#             encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=encoder_mask, encoder_mask=encoder_mask, user_mask=encoder_input)
#         decoder_status = self.reduce(*encoder_hidden)
#
#         encoder_features = self.w_h(encoder_outputs)
#
...
#                                            use_utter_trunc=self.use_utter_trunc, use_user_mask = self.use_user_mask)
#
#         if self.use_utter_trunc and self.use_user_mask:
...
#
#
# def _add_utter_pad(article, article_lens, max_len):
#     # print(article)
#     # print(article_lens)
#     # print(max_len)
#     article = [art[0] for art in article]
#     article_lens = [art_len[0] for art_len in article_lens]
#     max_len = [max_l[0] for max_l in max_len]
...
#         start += len_
#     utter_mask_ = utter_mask.unsqueeze(0)
#     return utter_mask_
...
class BertAttention(nn.Module):
    def __init__(self, hidden_dim, n_heads, dropout, eps, use_utter_trunc=False, use_user_mask=False):
        super(BertAttention, self).__init__()
        self.self = BertSelfAttention(hidden_dim, n_heads, dropout, use_utter_trunc, use_user_mask)
        self.output = BertSelfOutput(hidden_dim, dropout, eps)
        self.pruned_heads = set()
...
class TR_Encoder(nn.Module):
    def __init__(self, n_heads, encoder_lstm_num_layer, vocab_size, hidden_dim, intermediate_dim,
                 pad_idx, max_article_len, dropout, eps, use_utter_trunc=False, use_user_mask=False):
        super(TR_Encoder, self).__init__()
...
class LSTM_Encoder(nn.Module):
    def __init__(self, vocab_size, hidden_dim, pad_idx, encoder_lstm_num_layer, dropout):
    ...
    def forward(self, encoder_input, encoder_mask):
        ...
        seq_lens = encoder_mask.sum(dim=-1)
        packed = pack_padded_sequence(input=encoder_input, lengths=seq_lens, batch_first=True,
                                      enforce_sorted=False)
        output_packed, (h, c) = self.lstm(packed)
        output, _ = pad_packed_sequence(sequence=output_packed,
                                        batch_first=True,
...
class TR_Decoder(nn.Module):
    def __init__(self, n_heads, decoder_lstm_num_layer, hidden_dim, vocab_size,
                 intermediate_dim, dropout, eps, use_pointer, use_coverage, use_utter_trunc=False):
        super(TR_Decoder, self).__init__()
...
class LSTM_Decoder(nn.Module):
    def __init__(self, vocab_size, hidden_dim, pad_idx, decoder_lstm_num_layer, dropout, use_pointer=True, use_coverage=True):
    ...
    def forward(self, decoder_input_one_step, decoder_mask_one_step, decoder_status, encoder_output, encoder_features,
                encoder_mask, context_vec, oovs_zero, encoder_with_oov, coverage, step):
        decoder_embed_one_step = self.embedding(decoder_input_one_step)
        x = self.get_lstm_input(torch.cat([context_vec, decoder_embed_one_step], dim=-1)).unsqueeze(
            dim=1)
        ...
        status = torch.cat([h_t_reshape, c_t_reshape], dim=-1)
...
class PointerGeneratorNetworks(pl.LightningModule):
    def __init__(self, opt):
        super(PointerGeneratorNetworks, self).__init__()
        self.all_mode = ["train", "valid", "decode"]
        self.use_transformer_block = opt.use_transformer_block
        if self.use_transformer_block:
            self.encoder = TR_Encoder(opt.n_heads, opt.encoder_lstm_num_layer, opt.vocab_size, opt.hidden_dim, opt.intermediate_dim,
                                       opt.pad_idx, opt.max_article_len, opt.dropout, opt.eps, opt.use_utter_trunc, opt.use_user_mask)
            self.decoder = TR_Decoder(opt.n_heads, opt.decoder_lstm_num_layer, opt.hidden_dim, opt.vocab_size,
                                       opt.intermediate_dim, opt.dropout, opt.eps, opt.use_pointer, opt.use_coverage)
        else:
            self.encoder = LSTM_Encoder(opt.vocab_size, opt.hidden_dim, opt.pad_idx, opt.encoder_lstm_num_layer, opt.dropout)
            self.decoder = LSTM_Decoder(opt.vocab_size, opt.hidden_dim, opt.pad_idx, opt.decoder_lstm_num_layer, opt.dropout,
                                        opt.use_pointer, opt.use_coverage)
    ...
    def forward(self, encoder_input, encoder_mask, encoder_with_oov,
        ...
        if self.use_transformer_block:
            if mode in ["train", "valid"]:
                ...
                model_loss = self.tr_forward(encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec,
                                           coverage, decoder_input, decoder_mask, decoder_target)
                # print(model_loss)

                return model_loss
            elif mode in ["decode"]:
                return self.tr_decode(encoder_input=encoder_input, encoder_mask=encoder_mask,
                                     encoder_with_oov=encoder_with_oov, oovs_zero=oovs_zero, context_vec=context_vec,
                                     coverage=coverage, beam_size=beam_size)
        else:
            if mode in ["train", "valid"]:
                model_loss = self.lstm_forward(encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec,
                                           coverage, decoder_input, decoder_mask, decoder_target)

                return model_loss
            elif mode in ["decode"]:
                return self.lstm_decoder(encoder_input=encoder_input, encoder_mask=encoder_mask,
                                     encoder_with_oov=encoder_with_oov, oovs_zero=oovs_zero, context_vec=context_vec,
                                     coverage=coverage, beam_size=beam_size)


    def tr_forward(self, encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec, coverage,
                 decoder_input, decoder_mask, decoder_target, utter_mask=None, user_mask=None):

        assert isinstance(self.encoder, TR_Encoder)
        if self.use_utter_trunc or self.use_user_mask:
            encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=utter_mask, encoder_mask=encoder_mask, user_mask=user_mask)
        else:
            encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=encoder_mask, encoder_mask=encoder_mask, user_mask=user_mask)
    ...
    def tr_decode(self, encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec, coverage, utter_mask=None, user_mask=None, beam_size=4):
        if self.use_utter_trunc or self.use_user_mask:
            encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=utter_mask, encoder_mask=encoder_mask, user_mask=user_mask)
        else:
            encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, utter_mask=encoder_mask, encoder_mask=encoder_mask, user_mask=encoder_input)
    ...
    def lstm_forward(self, encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec, coverage,
                 decoder_input, decoder_mask, decoder_target):
        encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, encoder_mask=encoder_mask)
    ...
    def lstm_decode(self, encoder_input, encoder_mask, encoder_with_oov, oovs_zero, context_vec, coverage, beam_size=4):
        encoder_outputs, encoder_hidden = self.encoder(encoder_input=encoder_input, encoder_mask=encoder_mask)
        decoder_status = self.reduce(*encoder_hidden)
        ...
        while step < self.max_title_len and len(result) < 4:
            ...
            final_dist, decoder_status, context_vec, attention_score, genera_p, next_coverage = \
                self.decoder(
                    decoder_embed_one_step=decoder_input_one_step,
                    decoder_mask_one_step=decoder_mask_one_step,
                    decoder_status=decoder_status,
                    encoder_output=encoder_outputs_expand,
                    encoder_features=encoder_features_expand,
    ...
    def validation_step(self, batch, batch_idx):
        batch = from_batch_get_model_input(batch, self.hidden_dim,
                                           use_pointer=self.use_pointer, use_coverage=self.use_coverage)
        #TODO modified inputs and model parameters
